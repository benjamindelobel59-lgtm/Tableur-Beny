      ...
      :isCraft?(viewCraftLists.length===0?<div style={{textAlign:"center",padding:"60px",color:T.muted}}>⚗️ Aucune liste</div>:
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(285px,1fr))",gap:8}}>
          {viewChars.map(char=>{
            ...
            const surcatInfo=char.surcat==="PVP"?{icon:"🏆",label:"PVP",color:T.pvp}:{icon:"🐉",label:"PVM",color:T.pvm};
            return(
            <div key={char.id} style={{background:T.surface,border:"1px solid "+T.border,borderRadius:3,overflow:"hidden",boxShadow:T.shadow}}>
              <div style={{height:2,background:catColor}}/>
              <div style={{padding:"10px 12px"}}>
                {/* Header : icône + nom + badge Partagé */}
                <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:8}}>
                  <div style={{display:"flex",alignItems:"center",gap:9}}>
                    ...
                    <div>
                      <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:2}}>
                        ...
                        <span style={{fontSize:9,fontWeight:700,padding:"1px 5px",borderRadius:2,background:surcatInfo.color+"14",color:surcatInfo.color,border:"1px solid "+surcatInfo.color+"30",fontFamily:"'Rajdhani',sans-serif",letterSpacing:1}}>{surcatInfo.icon} {surcatInfo.label}</span>
                      </div>
                  ...
                  {/* Badge Partagé bleu */}
                  <span style={{fontSize:9,fontWeight:700,padding:"2px 7px",borderRadius:2,background:"rgba(59,130,246,0.12)",color:"#3b82f6",border:"1px solid rgba(59,130,246,0.3)",fontFamily:"'Rajdhani',sans-serif",letterSpacing:1,flexShrink:0,whiteSpace:"nowrap"}}>🔗 PARTAGÉ</span>
                </div>
                {/* Stats CLASSE / LEVEL / MAX */}
                ...
                {/* État + Surcat + Zone — lecture seule */}
                <div style={{display:"flex",gap:4,alignItems:"center"}}>
                  <div style={{flex:1,background:ETAT_COLORS[char.etat]+"11",border:"1px solid "+ETAT_COLORS[char.etat]+"33",borderRadius:2,padding:"5px 6px",color:ETAT_COLORS[char.etat],fontWeight:700,fontSize:11,fontFamily:"'Rajdhani',sans-serif",letterSpacing:.5,textAlign:"center"}}>{char.etat}</div>
                  <div style={{background:T.panel,border:"1px solid "+T.border,borderRadius:2,padding:"5px 6px",color:T.text,fontWeight:700,fontSize:11,fontFamily:"'Rajdhani',sans-serif"}}>{char.surcat==="PVP"?"🏆 PVP":"🐉 PVM"}</div>
...
function AppInner({ themeId, setTheme }) {
  const T=useT();const fi=makeFi(T);
  const [session,setSession]=useState(null);const [mainTab,setMainTab]=useState("persos");const [authLoading,setAuthLoading]=useState(true);
  const [characters,setCharacters]=useState([]);const [loading,setLoading]=useState(false);
  const [sharedChars,setSharedChars]=useState([]); // persos partagés avec moi
  ...
  useEffect(()=>{if(session){loadCharacters();loadShareCount();loadSharedChars();}},[session]);
  ...
  const loadSharedChars=async()=>{
    const{data:shares}=await supabase.from("shares").select("*").eq("shared_with_email",session.user.email).in("share_type",["all","compte"]);
    if(!shares||!shares.length){setSharedChars([]);return;}
    const all=[];
    for(const share of shares){
      ...
      if(data)all.push(...data.map(c=>({...c,_shared:true,_ownerEmail:share.owner_email,_canEdit:share.can_edit,_shareOwner:share.owner_id})));
    }
    setSharedChars(all);
  };
  ...
  const handleSubmit=async()=>{
    if(!form.nom.trim())return showToast("Le nom est requis !","err");
    if(editingChar?._shared){return handleSubmitShared();}
    const id=typeof editingChar==="object"?editingChar?.id:editingChar;
    if(id){const oldChar=characters.find(c=>c.id===id);const{error}=await supabase.from("characters").update(form).eq("id",id);if(error)return showToast("Erreur modification","err");if(oldChar&&form.level>oldChar.level)fireWebhook("levelup",{...oldChar,...form},{oldLevel:oldChar.level,newLevel:form.level});showToast("Modifié ✓");}
    else{const{error}=await supabase.from("characters").insert([{...form,user_id:session.user.id}]);if(error)return showToast("Erreur ajout","err");fireWebhook("add",form);showToast("Ajouté ✓");}
  ...
  // Handlers pour les persos partagés (éditables)
  const handleSharedEtatChange=async(id,etat)=>{const{error}=await supabase.from("characters").update({etat}).eq("id",id);if(error)return showToast("Erreur","err");setSharedChars(p=>p.map(c=>c.id===id?{...c,etat}:c));showToast("État mis à jour ✓");};
  const handleSharedSurcatChange=async(id,surcat)=>{const{error}=await supabase.from("characters").update({surcat}).eq("id",id);if(error)return showToast("Erreur","err");setSharedChars(p=>p.map(c=>c.id===id?{...c,surcat}:c));showToast("Dossier → "+surcat+" ✓");};
  const openEditShared=(c)=>{setForm({compte:c.compte,nom:c.nom,classe:c.classe,level:c.level,level_max:c.level_max,etat:c.etat,frigost:c.frigost,surcat:c.surcat||"PVM"});setEditingChar({id:c.id,_shared:true});setShowForm(true);};
  const handleSubmitShared=async()=>{if(!form.nom.trim())return showToast("Le nom est requis !","err");const id=editingChar.id;const{error}=await supabase.from("characters").update(form).eq("id",id);if(error)return showToast("Erreur modification","err");setSharedChars(p=>p.map(c=>c.id===id?{...c,...form}:c));showToast("Modifié ✓");setShowForm(false);};
        ...
        {false&&(
          <div style={{marginTop:22}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10,padding:"8px 12px",background:"rgba(59,130,246,0.06)",border:"1px solid rgba(59,130,246,0.2)",borderRadius:3}}>
              <span style={{fontSize:13}}>🔗</span>
              <span style={{fontSize:10,fontWeight:700,color:"#3b82f6",textTransform:"uppercase",letterSpacing:2,fontFamily:"'Rajdhani',sans-serif"}}>Partagés avec moi</span>
              <span style={{fontSize:9,color:"#3b82f6",background:"rgba(59,130,246,0.12)",border:"1px solid rgba(59,130,246,0.25)",borderRadius:2,padding:"1px 6px",fontWeight:700,fontFamily:"'Rajdhani',sans-serif"}}>{sharedChars.length}</span>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(285px,1fr))",gap:8}}>
              {sharedChars.map(char=>{
                const catColor=CATEGORIES.find(cat=>cat.etats?.includes(char.etat))?.color||T.accent;
                const sc=surcats.find(s=>s.id===(char.surcat||"PVM"))||surcats[2];
                const canEdit=char._canEdit;
                return(
                <div key={char.id} style={{background:T.surface,border:"1px solid rgba(59,130,246,0.25)",borderRadius:3,overflow:"hidden",boxShadow:T.shadow,transition:"border-color .15s"}}
                  ...
                  <div style={{padding:"10px 12px"}}>
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:9}}>
                        ...
                        <div>
                          ...
                          <div style={{display:"flex",alignItems:"center",gap:5}}>
                            <span style={{fontSize:10,color:T.muted,fontFamily:"'DM Sans',sans-serif"}}>{char.compte||<span style={{fontStyle:"italic"}}>Sans compte</span>}</span>
                            <span style={{fontSize:8,fontWeight:700,padding:"1px 5px",borderRadius:2,background:"rgba(59,130,246,0.1)",color:"#3b82f6",border:"1px solid rgba(59,130,246,0.25)",fontFamily:"'Rajdhani',sans-serif",letterSpacing:1}}>🔗 {char._ownerEmail?.split("@")[0]}</span>
                          </div>
                        </div>
                      </div>
                      <div style={{display:"flex",gap:3,flexShrink:0}}>
                        {canEdit&&<button onClick={()=>openEditShared(char)} style={{background:T.panel,border:"1px solid "+T.border,borderRadius:2,padding:"4px 7px",color:T.textSub,cursor:"pointer",fontSize:11,fontFamily:T.font}}>✏️</button>}
                        {!canEdit&&<span style={{fontSize:9,fontWeight:700,padding:"2px 6px",borderRadius:2,background:"rgba(59,130,246,0.08)",color:"#3b82f6",border:"1px solid rgba(59,130,246,0.2)",fontFamily:"'Rajdhani',sans-serif",letterSpacing:1,alignSelf:"center"}}>👁️ Lecture</span>}
                      </div>
                    </div>
                    <div style={{display:"flex",gap:3,marginBottom:8}}>
                      {[{l:"CLASSE",v:char.classe,c:T.textSub},{l:"LEVEL",v:char.level,c:T.accent},{l:"MAX",v:char.level_max,c:T.accent}].map(s=>(<div key={s.l} style={{flex:1,background:T.panel,borderRadius:2,padding:"4px 5px",textAlign:"center",border:"1px solid "+T.border}}><div style={{fontSize:7,color:T.muted,textTransform:"uppercase",letterSpacing:1.5,marginBottom:1,fontFamily:"'Rajdhani',sans-serif"}}>{s.l}</div><div style={{fontWeight:700,fontSize:11,color:s.c,fontFamily:"'Rajdhani',sans-serif"}}>{s.v}</div></div>))}
                    ...
                    <div style={{display:"flex",gap:4,alignItems:"center"}}>
                      {canEdit
                        ?<select value={char.etat} onChange={e=>handleSharedEtatChange(char.id,e.target.value)} style={{flex:1,background:ETAT_COLORS[char.etat]+"11",border:"1px solid "+ETAT_COLORS[char.etat]+"33",borderRadius:2,padding:"5px 6px",color:ETAT_COLORS[char.etat],fontWeight:700,fontSize:11,outline:"none",fontFamily:"'Rajdhani',sans-serif",cursor:"pointer",letterSpacing:.5}}>{ETATS.map(e=><option key={e}>{e}</option>)}</select>
                        :<div style={{flex:1,background:ETAT_COLORS[char.etat]+"11",border:"1px solid "+ETAT_COLORS[char.etat]+"33",borderRadius:2,padding:"5px 6px",color:ETAT_COLORS[char.etat],fontWeight:700,fontSize:11,fontFamily:"'Rajdhani',sans-serif",textAlign:"center",letterSpacing:.5}}>{char.etat}</div>
                      }
                      {canEdit
                        ?<select value={char.surcat||"PVM"} onChange={e=>handleSharedSurcatChange(char.id,e.target.value)} style={{background:T.panel,border:"1px solid "+T.border,borderRadius:2,padding:"5px 6px",color:T.text,fontWeight:700,fontSize:11,outline:"none",fontFamily:"'Rajdhani',sans-serif",cursor:"pointer"}}><option value="PVM">🐉 PVM</option><option value="PVP">🏆 PVP</option></select>
                        :<div style={{background:T.panel,border:"1px solid "+T.border,borderRadius:2,padding:"5px 6px",color:T.text,fontWeight:700,fontSize:11,fontFamily:"'Rajdhani',sans-serif"}}>{char.surcat==="PVP"?"🏆 PVP":"🐉 PVM"}</div>
                      }
                      <div style={{background:char.frigost!=="Continent"?T.accentBg:T.panel,border:"1px solid "+(char.frigost!=="Continent"?T.accentBorder:T.border),borderRadius:2,padding:"5px 7px",fontSize:11,color:char.frigost!=="Continent"?T.accent:T.muted,fontWeight:700,whiteSpace:"nowrap",fontFamily:"'Rajdhani',sans-serif"}}>{char.frigost!=="Continent"?`❄️ ${char.frigost==="Frigost 2"?"F2":char.frigost}`:"🌍"}</div>
                    </div>
