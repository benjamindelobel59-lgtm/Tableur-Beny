...
function CraftTab({ session, externalItems, onExternalConsumed, sharedCraftLists, sharedCraftOwner, sharedCraftCanEdit, onClearShared }) {
  ...
  const saveList=async()=>{if(!listName.trim())return showToast("Donne un nom !","err");if(craftItems.length===0)return showToast("Ajoute des items d'abord !","err");setSaving(true);if(updateId){const{error}=await supabase.from("craft_lists").update({name:listName.trim(),items:craftItems,banque,updated_at:new Date().toISOString()}).eq("id",updateId);setSaving(false);if(error)return showToast("Erreur : "+error.message,"err");showToast("Mise à jour ✓");}else{const{error}=await supabase.from("craft_lists").insert([{user_id:session.user.id,name:listName.trim(),items:craftItems,banque}]);setSaving(false);if(error)return showToast("Erreur : "+error.message,"err");showToast("Sauvegardée ✓");}loadSavedLists();};
  ...
  const confirmSaveModal=async()=>{const name=saveModalName.trim();if(!name)return showToast("Donne un nom !","err");setSaving(true);const{error}=await supabase.from("craft_lists").insert([{user_id:session.user.id,name,items:craftItems,banque}]);setSaving(false);if(error)return showToast("Erreur : "+error.message,"err");showToast(`"${name}" sauvegardée ✓`);setShowSaveModal(false);loadSavedLists();};
  ...
  const loadList=(list)=>{setActiveList(list);setCraftItems(list.items||[]);setListName(list.name);setUpdateId(list.id);if(list.banque)setBanque(list.banque);showToast(`"${list.name}" chargée ✓`);};
